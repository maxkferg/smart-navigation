...
class Universe:
    ...
    def addPenalty(self, x1, y1, x2, y2):
        """Add a rectangular penalty region specified by x/y coordinates"""
        self.penalties.append((x1,y1,x2,y2))


    def addSpawn(self, x1, y1, x2, y2):
        """Add a spawn locations specified by x/y coordinates"""
        self.spawn.append((x1,y1,x2,y2))
    ...
    def reset(self):
        spawn_spots = copy.copy(self.spawn)
        random.shuffle(spawn_spots)

        for particle in self.particles:
            rect = spawn_spots.pop()
            particle.x = random.uniform(rect[0]+particle.size, rect[0]+rect[2]-particle.size)
            particle.y = random.uniform(rect[1]+particle.size, rect[1]+rect[3]-particle.size)
            particle.speed = 15*random.random()

        for target in self.targets:
            rect = spawn_spots.pop()
            target.x = random.uniform(rect[0]+target.radius, rect[0]+rect[2]-target.radius)
            target.y = random.uniform(rect[1]+target.radius, rect[1]+rect[3]-target.radius)
    ...
    def isOnPenalty(self, particle):
        for penalty in self.penalties:
            ...
            if particle.x > x1 and particle.x > x2: # To the right
                continue
            elif particle.x < x1 and particle.x < x2: # To the left
                continue
            elif particle.y > y1 and particle.y > y2: # Above
                continue
            elif particle.y < y1 and particle.y < y2: # Below
                continue
            else:
                return True
        return False
